...
output: 
    pdf_document:
        extra_dependencies: ["amsmath", "amssymb", "fancyhdr"]
        toc: true
        toc_depth: 2
...
butane <- 1
oxygen <- 6.5*butane # part 1
carbondioxide <- 4*butane # part 3
water <- 5*butane # part 4
...
nitrogen <- 79/21*oxygen
excess <- (50/100*oxygen)+oxygen # 50% excess, part 2
...
```{r, attr.source='.numberLines'}
## reactants and products
methanol <- 1000/(12*1+4*1+16*1) # kg to moles
...
p_moles <- 90/100*moles # reacted moles
...
mass <- c(500, 4.5*weight[2], 4.5*weight[3]) # mass
...
oxygen <- ethylene/2
ethy_o <- (94.12/100)*ethylene # 94.12% yield
...
excess <- (20/100*oxygen) + oxygen # 20% excess, part 2
air <- (100/21)*excess # part 3
nitrogen <- (79/21)*excess
oxygen_at_end <- excess - oxygen # excess - theoretical
...
Percentage of CO~2~ is: **`r carbondioxide*100/final`%**  
Percentage of O~2~ (excess) is: **`r oxygen_at_end*100/final`%**  
Percentage of H~2~O is: **`r water*100/final`%**  
Percentage of N~2~ is: **`r nitrogen*100/final`%**  
...
Assuming for 100 kmol of composition gas:
...
## for 80% conversion
butane <- 80/100*butane
oxygen <- butane*6.5
carbondioxide <- butane*4
water <- butane*5

oxygen_at_end <- excess - oxygen # excess - theoretical
final <- (oxygen_at_end + carbondioxide + water
    + nitrogen + (100/80*butane-butane)) # part 3
```

Mole ratio of air to butane for complete combustion is: **`r air` : `r butane*100/80`**  

For incomplete combustion, the reaction is:

\chemeq{C_{4}H_{10} + 4.5O_{2} \rightarrow 4CO + 5H_{2}O}

The elements of combustion gas are: **CO, H~2~O, O~2~ (excess) and N~2~**

For 80% combustion of butane,

Percentage of CO~2~ is: **`r carbondioxide*100/final`%**  
Percentage of H~2~O is: **`r water*100/final`%**  
Percentage of butane is: **`r (100/80*butane-butane)*100/final`%**  
Percentage of N~2~ is: **`r nitrogen*100/final`%**  
Percentage of O~2~ is: **`r oxygen_at_end*100/final`%**  
